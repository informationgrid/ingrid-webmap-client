...
public class JsonCallbackResource{
	...
	@Consumes(MediaType.TEXT_PLAIN)
	@Produces(MediaType.APPLICATION_JSON)
	public Response searchAll(@QueryParam("searchTerm") String searchTerm, @QueryParam("json_callback") String jsonCallback, @QueryParam("data") String data) throws IOException, JSONException {
		...
		for (int i=0; i < cmps.length(); i++) {
			...
			URL questUrl = new URL(url.concat("&" + identifier+"="+URLEncoder.encode(searchTerm.trim())));
