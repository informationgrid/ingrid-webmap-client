...
public class ConvertConfig {
        ...
        if (file.exists()) {
            ...
            if(errors.size() > 0){
                for (int i = 0; i < errors.size(); i++) {
                    System.out.println( errors.get( i ) + " (" + i+1 + ")");
                }
            }else{
    ...
    private static void addLayerNode(Node node, String version, String url, String wmsLayers, String label, String layerBodId) throws JSONException {
        ...
        if( wmsLayers != null){
            ...
            String[] urlSplit = url.toLowerCase().split( "\\?" );
            ...
            if(urlSplit.length > 0){
                ...
                for (int i = 0; i < paramsSplit.length; i++) {
                    ...
                    if(param.startsWith( "version" ) || param.startsWith( "service" ) || param.startsWith( "request" ) || param.startsWith( "format" )){
        ...
        if (XPathUtils.nodeExists( node, xPath )) {
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                if (XPathUtils.nodeExists( subNode, "./name" )) {
                    ...
                    topic.put( "defaultBackground", "sg_geodatenzentrum_de_wms_webatlasde_light_webatlasde_light" );
