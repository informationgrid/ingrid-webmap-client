...
Ext.define('de.ingrid.mapclient.frontend.Workspace', {
    ...
    save: function(isTemporary, title, description) {
        ...
        var selectedLayersByService = this.activeServicesPanel.selectedLayersByService; 
        var treeState = this.activeServicesPanel.treeState;
        var activeServices = this.activeServicesPanel.getServiceList();
        
        // Clean up session
        if(this.activeServicesPanel.state){
            if(this.activeServicesPanel.state.capabilitiesUrlOrder){
                if(activeServices.length != this.activeServicesPanel.state.capabilitiesUrlOrder.length){
                    for(var j=0; j < treeState.length; j++){
                        var isExist = false;
                        for (var i=0; i < activeServices.length; i++){
                            var activeService = activeServices[i];
                            var capabilitiesUrl = activeService.capabilitiesUrl;
                            if(capabilitiesUrl){
                                if(treeState[j].capabilitiesUrl){
                                    if(treeState[j].capabilitiesUrl == capabilitiesUrl){
                                        isExist = true;
                                        break;
                                    } 
                                }
                    ...
                    for(var j=0; j < selectedLayersByService.length; j++){
                        var isExist = false;
                        for (var i=0; i < activeServices.length; i++){
                            var activeService = activeServices[i];
                            var capabilitiesUrl = activeService.capabilitiesUrl;
                            if(capabilitiesUrl){
                                if(selectedLayersByService[j].capabilitiesUrl){
                                    if(selectedLayersByService[j].capabilitiesUrl == capabilitiesUrl){
                                        isExist = true;
                                        break;
                                    } 
                                }
                        ...
                        if(isExist == false){
                            selectedLayersByService.splice(j, 1);
                            if(selectedLayersByService.length > 0){
                                j--;
                            }
                        }
                    }
        ...
        var data = new de.ingrid.mapclient.frontend.data.SessionState({
            ...
            selectedLayersByService: selectedLayersByService,
        ...
        this.session.load(state, shortUrl, {
            success : function(responseText) {
                ...
                state.restoreMapState(function() {
                    ...
                    // Remove not existing layers
                    if(self.map){
                        if(self.map.layers){
                            var mapLayers = self.map.layers;
                            if(state.activeServices){
                                var syncLayers = [];
                                var stateServices = state.activeServices;
                                if(stateServices){
                                    stateServices.forEach(function(stateService) {
                                        if(stateService.layers){
                                            if(stateService.layers.items){
                                                stateService.layers.items.forEach(function(stateServiceLayer) {
                                                    syncLayers.push(stateServiceLayer);
                                                }); 
                                            }
                                        }
                                    });
                                ...
                                if(syncLayers.length > 0){
                                    if(syncLayers.length != mapLayers.length){
